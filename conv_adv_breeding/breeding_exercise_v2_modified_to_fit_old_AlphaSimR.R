...
BP.eval <- function(va, vae, vres, ca, ew, BP, nsim=10){
  ...
  SP$addSnpChip(nSnpPerChr=genlen*10000/(1/snp.pct))
...
BP.plot <- function(out=out, threshold=c(3, 3, 3), nsim=10){
  ...
  # rename the column names (only for this old AlphaSimR compatible mode).
  for(i in 1:nbp){
    for(j in stage){
      colnames(out[[i]][[j]])[2:4] <- trait
    }
  }
...
BP.check <- function(out=out, threshold=c(3, 3, 3)){
  ...
  # rename the column names (only for this old AlphaSimR compatible mode).
  for(i in 1:nbp) colnames(out2[[i]])[2:4] <- c("size", "color", "shape")
